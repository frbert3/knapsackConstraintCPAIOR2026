// This is aglorithm 1 which enforces LAG on MK
// It is the version where we have Update&Flag(i,Q_{i0}) and Update&Flag(i,Q_{i1})
...
import Projectors.Projector;


public class PropKP2StepsProjectionOnQi1Qi0 extends Propagator<IntVar> {
    ...
    public PropKP2StepsProjectionOnQi1Qi0(BoolVar[] X, IntVar P, int[] v, int[] c, int[][] W, double threshold, int NOS, int nsteps){
...
//            Arrays.fill(lag, 0.0);
            ...
            if (NOS > 0) {
                local_alteration(0); // Algorithm 1 Line 14 Update&Flag(i, Q_{i0})
                local_alteration(1); // Algorithm 1 Line 15 Update&Flag(i, Q_{i1})
...
//            violation_of_constraint = (weights0 > c[j]) && (weights1 > c[j]) ;//;
    ...
    private void local_alteration(int side) {
        int k_prime;
        int K_prime = NOS;
        double saved_V = V;
        saved_sate();
        ...
        for(int i=0; i < n; i++) {
            if((!X[i].isInstantiated() && lagrangian_forbidden_value[i].size()==0)) {
                ...
                while (l<=nsteps && ((V - Math.abs(rc[i]))>=P.getLB())) { // First k step
                    ...
                    for(int j = 0; j < m; j++) {
                        lag[j] =  lag[j] - coef * grad[j]; // here it is a '-' bc we search a minimum.
                        rci += W[j][i] * lag[j];
                    }
                    P_C = new Projector(m, W, v);
                    lag = P_C.P_C(side, i, lag); // argmin{|l - l'|, l' in Q \cap R^m_+}
                    rci = v[i] - rci;
                    compute_RC();
...
//                        }
                    ...
                    while (k_prime < K_prime && goOn_local) {
                        ...
                        goOn_local = (lagrangian_forbidden_value[i].size() == 0) && update_lag_Qi(k_prime, 5, 15.0, i, side);
                        k_prime++;
                    }
...
//                        }
    ...
    private void compute_RC(){
        ...
        for(int i = 0; i < n; i++) {
//            if(iter==38) {
//                System.out.print("rc["+i+"]  = v["+i+"] + ");
//            }
            for(int j = 0; j < m; j++){
//                if(iter==38 && W[j][i] !=0) {
//                    System.out.print((-W[j][i])+"λ["+j+"] + ");
//                }
                rc[i] += W[j][i] * lag[j];
            }
            rc[i] = v[i] - rc[i];
...
//            }
    ...
    private boolean update_lag_Qi(int k, int K, double mu0, int idx, int side){
        int val_to_compare = P.getLB();
        if(is_grad_ok_at_best) {
            val_to_compare = Math.max(val_to_compare, best_int_v_grad);
        }
